...
func (sc *ServiceControl) RunMainService() error {
	sc.mu.Lock()
	defer sc.mu.Unlock()

	// 检查主服务是否已经启动
	if len(sc.CheckProcess()) >= 15 {
...
func (sc *ServiceControl) CheckProcess() []string {
...
// 停止服务
func (sc *ServiceControl) StopMainService() (err error) {
	sc.mu.Lock()
	defer sc.mu.Unlock()
	// 检查主服务是否已经启动
	if len(sc.CheckProcess()) == 0 {
	...
	// 清理文件的find命令
	dnfDir := "/home/neople/" // 替换为实际的DNF_DIR路径
	findCommands := [][]string{
